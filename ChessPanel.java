...
public class ChessPanel extends JPanel {
	...
	public int[][] pre = new int[drawn_num + 1][2]; // 记录下棋点的x,y坐标 最多 (drawn_num
													// + 1) 个
	...
	public ChessPanel(ImageIcon r_map, ImageIcon r_blackchess,
			ImageIcon r_whitechess) {
	...
	private void paintChessMap(Graphics g) { // 画棋盘
		...
		for (j = 0; j < N; j++) { // 画线
			g.drawLine(h / N / 2, h / N * j + h / N / 2, w - w / N + (N % 2)
					* (h / N / 2), h / N * j + h / N / 2);
			g.drawLine(w / N * j + h / N / 2, h / N / 2, w / N * j + h / N / 2,
					h - h / N + (N % 2) * (h / N / 2));
		}
	...
	public void putOne(int bwf) { // bwf 棋色 0:黑色 1：白色
		...
		for (int k = 0; k < bests.length; k++) {
			...
			y_max = temp4;
			// 差距小于1000，50%概率随机选取
			// System.out.println("外       :" + i + "," + j + "  mx:" + mx +
			// "  t:" + t);
		...
		for (int i = 0; i < 15; i++, System.out.print("\n"))
			for (int j = 0; j < 15; j++) {
				if (isChessOn[j][i] != 2)
					System.out.print(isChessOn[j][i]);
				else
					System.out.print(isChessOn[j][i]);
			}
		// 判断是否已分胜负
		...
		for (int i = i_min; i < i_max; i++)
			for (int j = j_min; j < j_max; j++)
				if (isChessOn[i][j] == 2) {
					...
					if (able_flag && bwf == 0
							&& (type_1 == 20 || type_1 == 21 || type_1 == 22)) // 禁手棋位置,不记录
						continue;
					rt[n][0] = i;
					rt[n][1] = j;
	...
	private int[] count(int x, int y, int ex, int ey, int bwf) {
		...
		int i;
		// 往正方向搜索
		for (i = 1; x + i * ex < 15 && x + i * ex >= 0 && y + i * ey < 15
				&& y + i * ey >= 0; i++) {
		...
		// 计算正方向活度,,
		// 最后一个位置不超过边界
		if (x + i * ex < 15 && x + i * ex >= 0 && y + i * ey < 15
				&& y + i * ey >= 0) {
			// 最后一个位置为空位 +1活
		...
		// 往反方向搜索
		for (i = 1; x - i * ex >= 0 && x - i * ex < 15 && y - i * ey >= 0
				&& y - i * ey < 15; i++) {
			if (isChessOn[x - i * ex][y - i * ey] == bwf)
				rt_2++;
			else if (isChessOn[x - i * ex][y - i * ey] == 2) {
		...
		// 两边都有中空
			...
			if (rt_1 + flag_i2 - 1 >= 4 || rt_2 + flag_i1 - 1 >= 4)
				return new int[] { 4, 1 };
			if (rt_1 + flag_i2 - 1 == 3 && ok_1 > 0 || rt_2 + flag_i1 - 1 == 3
					&& ok_2 > 0)
				return new int[] { 3, 2 };
			return new int[] { 3, 1 };
		}
		// 有一边有中空
			...
			// 多于5,先找成5,再找活4,剩下的只能是死4
				...
				if (flag_mid1
						&& (rt_2 + flag_i1 - 1 == 4 && ok_2 == 1 || flag_i1 == 4))
					return new int[] { 4, 2 };
				if (flag_mid2
						&& (rt_1 + flag_i2 - 1 == 4 && ok_1 == 1 || flag_i2 == 4))
					return new int[] { 4, 2 };
	...
	private Boolean makesense(int x, int y, int ex, int ey, int bwf) {
		...
		int rt = 1;
		for (int i = 1; x + i * ex < 15 && x + i * ex >= 0 && y + i * ey < 15
				&& y + i * ey >= 0 && rt < 5; i++)
		...
		for (int i = 1; x - i * ex >= 0 && x - i * ex < 15 && y - i * ey >= 0
				&& y - i * ey < 15 && rt < 5; i++)
			if (isChessOn[x - i * ex][y - i * ey] != 1 - bwf)
				rt++;
			else
	...
	protected int getType(int x, int y, int bwf) {
		...
		if (five_OR_more != 0)
			return 1; // 成5
		if (four_live != 0 || four_died >= 2 || four_died != 0
				&& three_live != 0)
